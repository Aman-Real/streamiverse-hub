...
import { getAuthNotice, getReturnPath, parseAuthMode } from "@/features/auth/utils/authNavigation";
...
const Auth = () => {
  ...
  return (
    <PageShell className="text-foreground">
      <main className="flex min-h-screen items-center justify-center px-4 pb-16 pt-24 sm:px-6 md:pt-28">
        {status === "signedOut" && (
          <section className="w-full max-w-md rounded-3xl border bg-card p-5 shadow-2xl sm:p-6 md:p-8">
            <AuthForm mode={mode} onModeChange={changeMode} notice={getAuthNotice(location.state)} />
          </section>
        )}
      </main>
    </PageShell>
