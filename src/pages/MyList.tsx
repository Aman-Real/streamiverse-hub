...
import {
  filterByType, formatLength, getContinueWatching, getResumeLabel, pickByIds, searchVideos, sortVideos, type SortKey,
...
const MyList = () => {
  ...
  const continueWatching = useMemo(() => getContinueWatching(videos), [videos]);
  // Resolve saved ids against the live library so scores and progress are always current.
  const saved = useMemo(
    () => sortVideos(searchVideos(filterByType(pickByIds(videos, myList.map(item => item.id)), filter), browser.search), sort),
    [videos, myList, filter, sort, browser.search],
  );
              ...
              aside={
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" onClick={event => event.stopPropagation()}>
                    ...
                    <DropdownMenuItem onSelect={() => updateProgress(video.id, 0)}>Remove from Continue Watching</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              }
            />
