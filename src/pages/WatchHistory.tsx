...
/** Everything the signed-in viewer has watched, finished titles included, most recent first. */
const WatchHistory = () => {
  const { history: watched, clearWatchHistory } = useWatchProgress();
          ...
          <div className="space-y-3">
            {watched.map(video => (
              <div key={video.id} className="bg-card border border-border rounded-lg p-4 flex items-center gap-4">
                <img src={video.backdrop ?? video.thumbnail} alt={video.title} className="w-28 h-16 object-cover rounded" />
