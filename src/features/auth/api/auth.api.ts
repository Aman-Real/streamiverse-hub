...
/** The signed-in account's uid right now, or null. For code outside React that must check who is signed in. */
...
/** Renames the signed-in Firebase account, so the session agrees with the profile document. */
export const updateAuthDisplayName = async (name: string) => {
